...
public class ShowPlayer extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ArrayList<MyPlayerModel> pitchers = new ArrayList<>();
			while(rs.next()) {
			...
			ArrayList<MyPlayerModel> catchers = new ArrayList<>();
			while(rs.next()) {
				MyPlayerModel player = new MyPlayerModel();
				player.setPlayerId(rs.getString("playerID"));
				player.setNumber(rs.getString("number"));
			...
			sql ="SELECT * FROM players as p " + 
					...
					"    ORDER BY number ASC";
			rs =  stmt.executeQuery(sql);
			ArrayList<MyPlayerModel> infielders = new ArrayList<>();
			while(rs.next()) {
				MyPlayerModel player = new MyPlayerModel();
				player.setPlayerId(rs.getString("playerID"));
				player.setNumber(rs.getString("number"));
			...
			sql ="SELECT * FROM players as p" + 
					...
					"    ORDER BY number ASC";
			rs =  stmt.executeQuery(sql);
			ArrayList<MyPlayerModel> outfielders = new ArrayList<>();
			while(rs.next()) {
				MyPlayerModel player = new MyPlayerModel();
				player.setPlayerId(rs.getString("playerID"));
				player.setNumber(rs.getString("number"));
		...
		//---------------------------------------
		request.getRequestDispatcher("ShowPlayer.jsp").forward(request, response);
	}
}
